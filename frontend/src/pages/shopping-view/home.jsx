        ...
        {featureImageList && featureImageList.length > 0 ? (
          featureImageList.map((slide, index) => (
            <img
              ...
              className={`${index === currentSlide ? "opacity-100" : "opacity-0"} absolute top-0 left-0 w-full h-full object-cover transition-opacity duration-1000 ease-in-out`} />
          ))
        ) : (
        ...
        {numImages > 1 && (
          <>
            <Button
              variant="outline"
              size="icon"
              onClick={() => setCurrentSlide((prev) => (prev - 1 + numImages) % numImages)}
            ...
            <Button
              variant="outline"
              size="icon"
              onClick={() => setCurrentSlide((prev) => (prev + 1) % numImages)}
      ...
      <section className="py-12 bg-muted/50">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {speciesWithIcon.map((speciesItem) => (
            <Card
              key={speciesItem.id}
              onClick={() => handleNavigateToListingPage(speciesItem, "species")}
            ...
            >
              <CardContent className="flex flex-col items-center justify-center p-6 aspect-square">
                ...
                <span className="font-semibold text-center">{speciesItem.label}</span>
              </CardContent>
            </Card>
          ))}
        </div>
      ...
      <section className="py-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {petList && petList.length > 0 ? (
            ...
            <p className="col-span-full text-center text-muted-foreground">No available pets found at this time.</p>
          )}
        </div>
      </section>
